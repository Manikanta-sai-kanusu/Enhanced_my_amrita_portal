...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_award') {
    ...
    if ($sid && $title) {
        $pdo->prepare('INSERT INTO awards (student_id, award_name, description, grace_marks_awarded, certificate_file, reviewed_by, review_status, award_date) VALUES (?,?,?,?,?,?,?,CURDATE())')->execute([$sid, $title, $desc, $grace, $file_path, $teacher_name, 'Approved']);
...
if (!empty($aids)) {
    ...
    $stmt = $pdo->prepare("SELECT a.*, s.name as student_name, s.enrollment_no FROM awards a JOIN students s ON a.student_id = s.id WHERE a.reviewed_by = ? ORDER BY a.award_date DESC");
            ...
            <?php else: ?>
                ...
                <div class="award-card">
                    <div style="display:flex; justify-content:space-between; align-items:flex-start; flex-wrap:wrap;">
                        <div>
                            <strong style="font-size:14px;"><?php echo htmlspecialchars($aw['award_name']); ?></strong>
                            <span class="badge badge-approved" style="margin-left:6px;"><?php echo htmlspecialchars($aw['review_status'] ?? 'Approved'); ?></span>
                            <div style="font-size:12px; color:#888; margin-top:4px;">
                                <i class="fa fa-user"></i> <?php echo htmlspecialchars($aw['student_name']); ?> (<?php echo htmlspecialchars($aw['enrollment_no']); ?>)
                                • Grace: <strong><?php echo floatval($aw['grace_marks_awarded']); ?></strong>
                                • <?php echo date('d M Y', strtotime($aw['award_date'] ?? date('Y-m-d'))); ?>
                            </div>
                        ...
                        <?php if ($aw['certificate_file']): 
                            $cert_path = $aw['certificate_file'];
                            if (strpos($cert_path, '/uploads/') === 0) {
                                $cert_path = '..' . $cert_path;
                            }
                        ?>
                        <div style="display:flex; gap:6px;">
                            <a href="<?php echo htmlspecialchars($cert_path); ?>" target="_blank" style="color:#2e7d32; font-size:11px; font-weight:600; padding:5px 12px; background:#e8f5e9; border-radius:6px; text-decoration:none;"><i class="fa fa-eye"></i> View</a>
                            <a href="<?php echo htmlspecialchars($cert_path); ?>" download style="color:#1565c0; font-size:11px; font-weight:600; padding:5px 12px; background:#e3f2fd; border-radius:6px; text-decoration:none;"><i class="fa fa-download"></i> Download</a>
                        </div>
                        <?php endif; ?>
                    </div>
                </div>
